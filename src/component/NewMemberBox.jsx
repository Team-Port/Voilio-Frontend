...
const NewMemberBox = () => {
  ...
  const getNewMembers = () => {
    axios
      .get(`${HOST_URL}/api/v1/users/list`)
      .then((response) => {
      ...
      <div className="subscribe-box">
        {newMembers.map((member, index) => (
          <div key={index} onClick={() => moveToUser(member.nickname)}>
            <img
              className="subscribeNum"
              src={process.env.PUBLIC_URL + `/asset/num${index + 1}.png`}
            />
            <img
              className="ss-channelImg"
              src={process.env.PUBLIC_URL + "/asset/test_thum.jpeg"}
            />
            {member.nickname}
          </div>
        ))}
